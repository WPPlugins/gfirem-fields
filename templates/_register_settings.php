<table class="form-table">
    ...
    <tr>
        <th>
            <label for="reg_password"><?php _e( 'Password', 'frmreg' ) ?>aaaa</label>
        </th>
        <td>
            <select name="<?php echo esc_attr( $this->get_field_name( 'reg_password' ) ) ?>" id="reg_password">
				...
				if ( isset( $fields ) and is_array( $fields ) ) {
					foreach ( $fields as $field ) {
						//TODO hay que estar revizando cuando cambien esta option por el filtro
						if ( in_array( $field->type, array( 'text', 'password', 'password_field', 'password_field' ) ) ) { ?>
    ...
    <tr>
        ...
        <td>
            <select name="<?php echo esc_attr( $this->get_field_name( 'reg_role' ) ) ?>" id="<?php echo esc_attr( $this->get_field_name( 'reg_role' ) ) ?>">
                <option value="subscriber"><?php _e( '- None -', 'frmreg' ) ?></option>
		        <?php
		        if ( isset( $fields ) and is_array( $fields ) ) {
			        foreach ( $fields as $field ) {
				        if ( in_array( $field->type, array( 'data', 'role_list') ) ) { ?>
                            <option value="<?php echo esc_attr( $field->id ) ?>" <?php selected( $form_action->post_content['reg_role'], $field->id ) ?>><?php echo substr( esc_attr( stripslashes( $field->name ) ), 0, 50 );
						        unset( $field );
						        ?></option>
					        <?php
				        }
	        ...
	        <?php _e( 'By default is Subscriber', 'frmreg' ) ?>
